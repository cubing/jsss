...
{
  const orientationKpuzzle = new KPuzzle(x3x3_kpuzzle_default);
  const uAlgs = ["", "z", "x", "z'", "x'", "x2"].map((s) => parseAlg(s));
  ...
  for (const uAlg of uAlgs) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      puzzleOrientationCache[idxU][idxL] = Invert(x3x3_kpuzzle_default, orientationKpuzzle.state);
    }
  }
}
function normalizePuzzleOrientation(s) {
  ...
  return Combine(x3x3_kpuzzle_default, s, orientationTransformation);
}
function reorientPuzzle(s, idxU, idxL) {
  const orientationTransformation = Invert(x3x3_kpuzzle_default, puzzleOrientationCache[idxU][idxL]);
  return Combine(x3x3_kpuzzle_default, s, orientationTransformation);
}
...
//# sourceMappingURL=chunk.TIPXB7XW.js.map
